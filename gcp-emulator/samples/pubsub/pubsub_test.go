...
const (
	testSub      = "in-sub1"
	testSubTopic = "in-topic1"
	resultTopic  = "out-topic1"
	resultSub    = "out-sub1"
)
...
func TestPubSubProcessing(t *testing.T) {
	...
	//subscribe first
	go func() {
		app.client.Topic(testSubTopic).Publish(ctx, &pubsub.Message{Data: []byte("{\"Content\" : \"Hello world Emulator!!\"}")})
		app.client.Subscription(resultSub).Receive(ctx, func(ctx context.Context, msg *pubsub.Message) {
			...
			assert.Equal(t, "Hello world Emulator!!", jsonMessage["Content"], "Content field is kept as is")
