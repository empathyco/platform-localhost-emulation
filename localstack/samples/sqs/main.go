...
var SQSEndpoint = "http://localstack:4566"
...
func main() {
    ...
    SQSEndpoint, exists := os.LookupEnv("LOCALSTACK_URL")
    if exists {
        fmt.Println(SQSEndpoint)
    }
    flag.Parse()
    ...
    for _, value := range msgResult.Messages {
        fmt.Println("Message ID:     " + *value.MessageId)
        fmt.Println("Body:     " + *value.Body)
        // snippet-start:[sqs.go.receive_messages.print_handle]
        fmt.Println("Message Handle: " + *value.ReceiptHandle)
    }
